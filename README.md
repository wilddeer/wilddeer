# Hey, I'm Oleg

Lead Engineer & Architect at [Brevo](https://www.brevo.com/), working on AI for [Brevo Conversations](https://www.brevo.com/products/conversations/). Based in Barcelona.

Dropped out of uni. Joined Getwear and reworked their website from scratch, but [it didn't last long](https://www.smashingmagazine.com/2015/11/lessons-learned-shutting-startup/). Then launched [Chatra](https://chatra.com/) with the same founders – a live chat we built in 5 months with a team of four. Worked my ass off for the success of the product. The product succeeded – Chatra was acquired by Brevo. Now I work at Brevo and make small indie things in my spare time.

## Some old open source of note

Back when browser compatibility was a real job, I made a few things people found useful:

| Project | ⭐ | |
|---------|---:|---|
| [stickyfill](https://github.com/wilddeer/stickyfill) | 2.3k | CSS `position: sticky` polyfill. Did its job, no longer needed. |
| [Peppermint](https://github.com/wilddeer/Peppermint) | 217 | Touch slider with mouse, Touch Events, and Pointer Events support |
| [Sniffer](https://github.com/wilddeer/Sniffer) | 181 | Browser, engine, OS, and device detection |

## Fresh stuff

- [specops](https://github.com/wilddeer/specops) – Opinionated LLM skills and subagents for spec-driven development, inspired by [Superpowers](https://github.com/obra/superpowers)

## Links

- Blog: [grumpy.blog](https://grumpy.blog/en/)
- LinkedIn: [oleg-korsunskiy](https://www.linkedin.com/in/oleg-korsunskiy/)
